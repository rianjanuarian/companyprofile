<!DOCTYPE html>
<html lang="en">
    <head>
    <script src="https://kit.fontawesome.com/4f7eb700fd.js" crossorigin="anonymous"></script>
        <meta charset= "UTF-8">
        <meta name="viewport" content="width=device-width,initial0scale=1.0">
        <meta http-equiv="X-UA-Compatible" content="ie=edge">
        <link rel = "stylesheet" href="index.css">
        <link rel = "stylesheet" href="carousel.css">
        <title> Alfa Tech Media</title>
</head>
<body>
    <header class="showcase">
        
        <div class= "content">
            <img src="images/alfatech.jpeg" class="logo" alt="Kentang Tech Media">
            <div class="title">
                Alfa Tech
</div>
<div class="text">
 
</div>
</div>
</header>
<section class="services">
    <div class="container grid-2 center">
        <div>
        <a href="katalog.php"> <i class="fa fa-briefcase fa-3x" aria-hidden="true"></i></a>
        <h3>Katalog</h3>
        <p> Katalog Komponen PC </p>
        </div>
        <div>
        <a href="https://www.tokopedia.com/alfatech223"> <i class="fa fa-briefcase fa-3x" aria-hidden="true"></i></a>
        <h3>Toko</h3>
        <p> Kunjungi Tokopedia Kami </p>
        </div>
    </div>
</section>



<section class="about bg-light">

<div class="container1">
  <div class="card-carousel">
    <div class="card" id="1">
      <div class="image-container"></div>
      <p>VGA Terbaik memberikan render terbaik untuk game anda</p>
    </div>
    <div class="card" id="2">
      <div class="image-container"></div>
      <p>Processor terbaik memberikan pengalaman terbaik dalam game anda</p>
    </div>
    <div class="card" id="3">
      <div class="image-container"></div>
      <p>Motherboard terbaik memberikan performa terbaik dari tiap komponen pc anda</p>
    </div>  
    <div class="card" id="4">
      <div class="image-container"></div>
      <p>Power Supply terbaik akan menjaga umur pemakaian komponen pc anda</p>
    </div>
    <div class="card" id="5">
      <div class="image-container"></div>
      <p>RAM terbaik akan memberikan pengalaman multitasking tanpa hambatan</p>
    </div>
  </div>
  <a href="#" class="visuallyhidden card-controller">Carousel controller</a>
</div>
<!-- 
<h2>Check out <a target="_blank" href="https://codepen.io/WillyW/pen/RwRrvjv">Circle Carousel</a></h2> -->
</section>
<section class="about bg-light">
    <div class="container">
        <div class="grid-2">
            <div class="center">
           <img src="images/alfatech.jpeg" height= "200px" width="200px">
            </div>
            <div>
                <h3>Tentang Kami </h3>
                <p> Dengan semakin berkembang dan tingginya kebutuhan masyarakat terhadap pelayanan yang berhubungan dengan kebutuhan perangkat keras komputer (hardware) dan perangkat lunak komputer (software), kami berinisiatif untuk membuka Toko dengan konsep kelas menengah untuk memberikan pengalaman berbelanja terbaik kepada para pelanggan. </p>
            </div>
        </div>
    </div>
</section>
<footer class="center bg-dark">
    <p> Alfa Tech Media</p>

</footer>


</body>
<script type="text/javascript" src="carousel.js"></script>
</html>